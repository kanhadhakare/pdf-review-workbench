...
if (!(await isPortOpen(4200))) {
  spawnDetached("frontend", "cmd.exe", ["/c", "npm.cmd", "run", "start", "--", "--host", "0.0.0.0"], path.join(root, "frontend"), "public-frontend.log");
} else {
  console.log("frontend already running on 4200");
}
