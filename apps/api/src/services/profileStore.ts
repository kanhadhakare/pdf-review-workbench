...
import { type ExtractionProfile, type ProfileSummary } from "@pdf-review-workbench/shared";
...
async function ensureRoot(): Promise<void> { await mkdir(PROFILES_ROOT, { recursive: true }); }
function getPath(fingerprint: string): string { return path.join(PROFILES_ROOT, `${fingerprint}.json`); }

export async function loadProfile(fingerprint: string): Promise<ExtractionProfile> {
  if (cache.has(fingerprint)) return cache.get(fingerprint)!;
  await ensureRoot();
  try {
    const content = await readFile(getPath(fingerprint), "utf8");
    const parsed = JSON.parse(content) as ExtractionProfile;
