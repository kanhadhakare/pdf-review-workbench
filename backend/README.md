...
- Install Docling in a separate backend Python environment before using production-like tagging detection:
  - Windows: `python -m venv .docling-venv && .\.docling-venv\Scripts\python -m pip install --upgrade pip docling`
  - Linux/macOS: `python3 -m venv .docling-venv && ./.docling-venv/bin/python -m pip install --upgrade pip docling`
- Set `DOCLING_PYTHON_BIN` if Docling is installed somewhere else.
