...
function normalizeResult(payload: unknown, fallbackError?: string): MathOcrResult {
...
function parseRunnerJson(stdout: string): unknown {
...
async function runPix2Text(command: string, imagePath: string): Promise<MathOcrResult & { launchFailed?: boolean }> {
...
async function recognizeEquationCropInternal(imagePath: string): Promise<MathOcrResult> {
  if (pythonCommand) {
...
export async function recognizeEquationCrop(imagePath: string): Promise<MathOcrResult> {
