...
import { type ExtractedFontAsset, type FontExtractionManifest } from "../types.js";
...
const CONVERTIBLE_FORMATS = new Set<ExtractedFontAsset["format"]>(["truetype", "opentype"]);
const REBUILDABLE_FORMATS = new Set<ExtractedFontAsset["format"]>(["type1", "unknown"]);

let cachedFontForgeCommand: string | null | undefined;

function align4(value: number): number {
  return (value + 3) & ~3;
}

function isSfntFont(bytes: Buffer): boolean {
...
async function resolveFontForgeCommand(): Promise<string | null> {
  if (cachedFontForgeCommand !== undefined) return cachedFontForgeCommand;
...
async function runProcess(command: string, args: string[]): Promise<{ ok: boolean; stdout: string; stderr: string }> {
...
function parseSfntTables(bytes: Buffer): SfntTable[] {
  if (!isSfntFont(bytes)) throw new Error("Font is not an SFNT font");
  const numTables = bytes.readUInt16BE(4);
  const tableDirEnd = 12 + (numTables * 16);
  if (numTables <= 0 || tableDirEnd > bytes.length) throw new Error("Invalid SFNT table directory");
  ...
  for (let index = 0; index < numTables; index += 1) {
    ...
    if (tableOffset + length > bytes.length) throw new Error("Invalid SFNT table bounds");
    tables.push({
...
function buildWoff(bytes: Buffer): Buffer {
...
function convertedFileName(fileName: string, extension: "woff" | "woff2"): string {
  return `${fileName.replace(/\.[^.]+$/, "")}.${extension}`;
}

function convertedAsset(source: ExtractedFontAsset, fileName: string, format: "woff" | "woff2"): ExtractedFontAsset {
  return {
    ...source,
...
function rebuiltFileName(fileName: string): string {
  return `${fileName.replace(/\.[^.]+$/, "")}-rebuilt.otf`;
}

async function rebuildFontWithFontForge(font: ExtractedFontAsset, sourcePath: string, targetPath: string): Promise<ExtractedFontAsset | null> {
...
export async function convertManifestFontsForWeb(jobId: string, manifest: FontExtractionManifest): Promise<FontExtractionManifest> {
  if (!manifest.fonts.length) return manifest;
  const fontsDir = jobStore.getFontsDir(jobId);
  const rebuiltFonts: ExtractedFontAsset[] = [];
  const convertedFonts: ExtractedFontAsset[] = [];
  ...
  for (const font of manifest.fonts) {
    ...
    if (!isSfntFont(sourceBytes) && REBUILDABLE_FORMATS.has(font.format)) {
      ...
      if (!rebuiltBytes || !isSfntFont(rebuiltBytes)) continue;
      rebuiltFonts.push(rebuilt);
      conversionSource = rebuilt;
      conversionBytes = rebuiltBytes;
    }

    if (!CONVERTIBLE_FORMATS.has(conversionSource.format) || !isSfntFont(conversionBytes)) continue;
  ...
  if (!rebuiltFonts.length && !convertedFonts.length) return manifest;
