...
function groupLines(blocks, tolerance) {
...
export function validatePage(blocks, profile, pageWidth, pageHeight) {
    if (!blocks.length || pageWidth <= 0 || pageHeight <= 0)
        return 0;
    ...
    const indentConsistencyScore = (() => {
        if (profile.firstLineIndentPx <= 0)
            return 0;
    ...
    const score = (baselineAlignmentScore * 0.30) +
...
export function pageConfidencePoints(page) {
    return { pageIndex: page.pageIndex, confidence: page.confidence };
}
export function applyOcrValidation(baseScore, ocrPage, comparison) {
