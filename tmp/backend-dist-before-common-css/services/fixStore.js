...
async function ensureDir(dirPath) { await mkdir(dirPath, { recursive: true }); }
async function readJson(filePath) { try {
...
export async function getAllFixes() {
    ...
    for (const file of files) {
        if (!file.endsWith(".json") || file.includes(`${path.sep}visits${path.sep}`))
            continue;
...
export async function getFixesByJob(jobId) {
    const fixes = await getAllFixes();
    return fixes.filter((fix) => fix.jobId === jobId);
}
export async function getVisitsByJob(jobId) {
...
export async function getFixCount() { return (await getAllFixes()).length; }
export async function buildEditSummary(jobId) {
    const summary = template(jobId);
    const fixes = await getFixesByJob(jobId);
    const visits = await getVisitsByJob(jobId);
    const sessions = new Set();
    for (const visit of visits) {
        if (!summary.pagesReviewed.includes(visit.pageIndex))
            summary.pagesReviewed.push(visit.pageIndex);
    ...
    for (const fix of fixes) {
        ...
        if (!summary.pagesEdited.includes(fix.pageIndex))
            summary.pagesEdited.push(fix.pageIndex);
        sessions.add(fix.reviewerId);
        if (!summary.lastEditAt || fix.timestamp > summary.lastEditAt)
            summary.lastEditAt = fix.timestamp;
    }
...
export async function hasFixesForJob(jobId) {
    try {
...
export async function deleteFixesForJob(jobId) {
